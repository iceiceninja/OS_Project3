 ...
 * @author iceiceninja
 */
public class ChainedAllocation implements IFileAllocationMethod {
...
//        diskDrive.writeBlock(1, );
    ...
    public void storeFile(String fileName, byte[] fileContents) {
         ...
         */
        byte[] bitmapContents = diskDrive.readBlock(Bitmap);
//        int blocksToAllocate = fileBlockLength;
        int[] blocksAllocated = new int[fileBlockLength+1];
        int allocatedBlockCounter = 0;
        for (int i = 0; i < bitmapContents.length; i++) {
            if (bitmapContents[i] == 0) {
//                blocksToAllocate -= 1;
                bitmapContents[i] = 1;
                blocksAllocated[allocatedBlockCounter] = i;
                allocatedBlockCounter++;
            }
        }
...
//        diskDrive.writeBlock(startBlock, fileContents);
        ...
        int blockCounter = 0;
        for(int currBlockindex = 0; currBlockindex<blocksAllocated.length; currBlockindex++)
        {
            int nextBlock = blocksAllocated.length > i+1 ? blocksAllocated[i+1] : 0;
            int currBlock = blocksAllocated[currBlockindex];
            byte[] blockToWrite = Arrays.copyOfRange(fileContents, blockCounter * 512, (blockCounter + 1) * 512);
            blockCounter++;
            byte[] updatedBlockToWrite = new byte[blockToWrite.length +1];
            updatedBlockToWrite[0] = (byte)nextBlock;
            
            System.arraycopy(blockToWrite, 0, updatedBlockToWrite, 1, blockToWrite.length);
            diskDrive.writeBlock(currBlock, updatedBlockToWrite);
